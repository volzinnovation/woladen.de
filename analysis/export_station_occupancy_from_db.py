    ...
    *,
    ...
    with store.connection() as conn:
        ...
        for row in daily_rows:
            matching_observations = int(row["matching_observations"] or 0)
            if matching_observations < minimum_observations:
                continue
            key = (str(row["provider_uid"]), str(row["station_id"]))
            archive_date = str(row["archive_date"])
            accepted_provider_station_days.add((key[0], key[1], archive_date))
            aggregate = aggregates[key]
            aggregate.observed_days.add(archive_date)
            try:
            ...
            aggregate.matching_observations += matching_observations
        ...
        for row in hourly_rows:
            key = (str(row["provider_uid"]), str(row["station_id"]))
            archive_date = str(row["archive_date"])
            if (key[0], key[1], archive_date) not in accepted_provider_station_days:
                continue
            aggregate = aggregates[key]
    ...
    if required_dates:
        aggregates = {
            ...
            if required_dates.issubset(aggregate.observed_days)
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-matching-observations-per-day",
        type=int,
        default=0,
        help="Only export stations with at least this many mapped status updates on every included day",
    )
...
def main() -> None:
    ...
    aggregates = _load_aggregates(
        ...
        end_date=end_date,
        min_matching_observations_per_day=args.min_matching_observations_per_day,
        required_dates=set(target_date_texts) if args.min_matching_observations_per_day > 0 else None,
    )
